...
def ImageReader(ImageFolderPath):
    Images = []  # Input Images will be stored in this list.

    # Checking if path is of folder.
    if os.path.isdir(ImageFolderPath):  # If path is of a folder contaning images.
        ImageNames = os.listdir(ImageFolderPath)   # It will return a list of all files and directories in the specified directory
        ImageNames_Split = [[int(os.path.splitext(os.path.basename(ImageName))[0]), ImageName] for ImageName in
                            ImageNames]
        ImageNames_Split = sorted(ImageNames_Split, key=lambda x: x[0])  #sorts the names of the images
...
def FindMatches(BaseImage, SecImage):
    # Using SIFT to find the key points in the images
...
if __name__ == "__main__":
    # Reading images.
    Images = ImageReader("C:\\Users\\Saloni\\PycharmProjects\\ImageMosaicking\\images")
    ...
    cv2.imwrite("Panorama.png", BaseImage)